...
import { bankStatementService as statementService } from '../../../services/bankStatementService';
...
const StatementPreview = () => {
  ...
  const handleFetchStatement = async () => {
    ...
    try {
      setLoading(true);
      const stmt = await statementService.generateStatement(filters.accountId, filters.fromDate, filters.toDate, 'PDF');
      setData(stmt);
  ...
  const handleDownload = async (formatType) => {
    try {
      setDownloading(true);
      toast.loading(`Preparing ${formatType}...`, { id: 'download' });
      await statementService.downloadStatement(formatType);
      toast.success(`${formatType} downloaded successfully!`, { id: 'download' });
    } catch (err) {
      toast.error(`Failed to download ${formatType}`, { id: 'download' });
    } finally {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 print:hidden">
        <div className="flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="w-full md:w-1/3">
            ...
            {filterErrors.accountId && <p className="text-xs text-red-500 mt-1">{filterErrors.accountId}</p>}
          </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
              ...
              <div className="flex justify-between items-center text-green-600 text-sm">
                <div className="flex items-center gap-1"><ArrowDownRight className="w-4 h-4"/> Total Credits</div>
                <span className="font-semibold">+ {formatCurrency(data.summary.totalCredits)}</span>
              </div>
              <div className="flex justify-between items-center text-red-500 text-sm">
                <div className="flex items-center gap-1"><ArrowUpRight className="w-4 h-4"/> Total Debits</div>
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={trendData}>
                    ...
                    <YAxis tick={{fontSize: 12}} axisLine={false} tickLine={false} tickFormatter={(val) => `${currencySymbol}${val/1000}k`} />
                    <RechartsTooltip formatter={(value) => formatCurrency(value)} />
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden">
                ...
                <div className="flex justify-between items-center gap-3">
                  <button onClick={handlePrint} className="btn-secondary flex items-center gap-2 bg-white py-1.5 text-xs">
                    <Printer className="w-3.5 h-3.5" /> Print
                  </button>
                  <button onClick={() => handleDownload('CSV')} disabled={downloading} className="btn-secondary flex items-center gap-2 bg-white py-1.5 text-xs">
                    <FileText className="w-3.5 h-3.5" /> CSV
                  </button>
                  <button onClick={() => handleDownload('PDF')} disabled={downloading} className="btn-primary flex items-center gap-2 py-1.5 text-xs shadow-sm">
